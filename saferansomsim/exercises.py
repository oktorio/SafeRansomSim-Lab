"""Fixed-root SOC/IR exercise preparation, replay, and scoring."""
...
def load_dataset(scenario_id: str) -> list[dict[str, Any]]:
    ...
    if not path.is_file():
        raise SafetyError(f"Bundled synthetic dataset missing for scenario: {scenario_id}")
...
def prepare_exercise(scenario_id: str) -> Path:
    ...
    if root.exists():
        ...
        if any(root.iterdir()):
            raise SafetyError(
                f"Exercise {scenario_id!r} already contains artifacts; refusing to overwrite analyst work."
            )
    else:
...
def _owned_score_file(path: Path, scenario_id: str) -> bool:
    ...
    if path.suffix == ".json":
        ...
        return (
            ...
            and payload.get("project") == config.PROJECT_NAME
    ...
    try:
        ...
        return False
    return (
        f"data-project=\"{config.PROJECT_NAME}\"" in text
        and f"data-scenario=\"{scenario_id}\"" in text
    )
